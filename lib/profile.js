...
const prepareSearchQuery = (searchQuery) => {
    ...
    if (searchQuery.skills) {
        console.log("search the skilllllssss", searchQuery.skills)
        var skillsQuery = { $and: [] };
        //skillsQuery['$and'].push({ "skills": { '$in': { '$regex': searchQuery.skills, $options: 'i' } } });
        // searchQuery.skills.map(skill => {
        //     skillsQuery['$and'].push({ "skills": { '$in': { '$regex': skill, $options: 'i' } } })
        // })
